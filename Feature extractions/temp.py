...
class_labels = pd.read_csv("C:\\Users\\arpit\\OneDrive\\Desktop\\AI\\Extracted datasets\\labels.csv",encoding='utf-8')
weighted_tfidf_score = pd.read_csv('C:\\Users\\arpit\\OneDrive\\Desktop\\AI\\Extracted datasets\\tfidf_scores.csv',encoding='utf-8')
sentiment_scores = pd.read_csv('C:\\Users\\arpit\\OneDrive\\Desktop\\AI\\Extracted datasets\\sentiment_scores.csv',encoding='utf-8')
dependency_features = pd.read_csv('C:\\Users\\arpit\\OneDrive\\Desktop\\AI\\Extracted datasets\\dependency_features.csv',encoding='utf-8')
word_bigrams = pd.read_csv('C:\\Users\\arpit\\OneDrive\\Desktop\\AI\\Extracted datasets\\word_bigram_features.csv',encoding='utf-8')
tfidf_sparse_matrix = pd.read_csv('C:\\Users\\arpit\\OneDrive\\Desktop\\AI\\Extracted datasets\\tfidf_features.csv',encoding='utf-8')

#merge all features
df_list=[class_labels,sentiment_scores,weighted_tfidf_score,dependency_features,word_bigrams,tfidf_sparse_matrix]
master = df_list[0]
for df in df_list[1:]:
    master = master.merge(df, on='index')

#ignore first two columns (index and tweet)
y=master.iloc[:20000,2] #class labels
X=master.iloc[:20000,3:] #all features
...
# scaled version of training and test data
...
#Base training set to benchmark performance (weighted tif-df score as only feature)

x_base = pd.DataFrame(X_train.loc[:,'weighted_TFIDF_scores'])
x_base_test = pd.DataFrame(X_test.loc[:,'weighted_TFIDF_scores'])

#initialize models
lr = LogisticRegressionCV(multi_class='ovr', solver='sag',cv=3,max_iter=6000)
gb = GradientBoostingClassifier(n_estimators=500, learning_rate=.025)
xgb = XGBClassifier(learning_rate=.025)
mlp = MLPClassifier(solver='sgd',max_iter=4000,hidden_layer_sizes=(80,40,40,10), activation='relu', random_state=1,learning_rate='adaptive', alpha=1e-6)
rf= RandomForestClassifier(n_estimators=100,max_features=500)


from sklearn.model_selection import cross_val_score

print("Baseline model f1-score = ", cross_val_score(lr,x_base, y_train,scoring="f1_micro",cv=3).mean()) #benchmark model: linear regression using just tfidf score (weighted with hate dict)
